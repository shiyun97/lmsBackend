    ...
    public Response completeCoursepack(@QueryParam("userId") Long userId, @QueryParam("coursepackId") Long coursepackId) {
        try {
            ...
            for (Outlines o : outlinesList) {
                ...
                //Long i = Long.valueOf(lessonOrderList.size());
                for (LessonOrder l : lessonOrderList) {
                    Quiz quiz = l.getQuiz();
                    List<QuizAttempt> quizAttemptList = quiz.getQuizAttemptList();
                    if (quizAttemptList == null || quizAttemptList.isEmpty()) {
                        return Response.status(Status.NOT_FOUND).entity(new ErrorRsp("No quiz attempt found!")).build();
                    }
                    for (QuizAttempt q : quizAttemptList) {
                        if (q.getQuizTaker() == user) {
                            l.getPublicUserList().add(user);
                        }
                    }
                }
                /*(Quiz quiz = lastLessonOrder.getQuiz();
                List<QuizAttempt> quizAttemptList = quiz.getQuizAttemptList();
                if (quizAttemptList == null || quizAttemptList.isEmpty()) {
                    return Response.status(Status.NOT_FOUND).entity(new ErrorRsp("No quiz attempt found!")).build();
                }
                for(QuizAttempt q : quizAttemptList){
                    if(q.getTotalMarks().equals(quiz.getMaxMarks())){
                        lastLessonOrder.getPublicUserList().add(user);                        
                    }
                }*/
            }
